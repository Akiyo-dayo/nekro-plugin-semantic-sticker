...
- **戳一戳回复**：收到 OneBot V11 poke 事件后直接检索并发送一张表情包；事件同时继续传播给 NekroAgent 核心通知管道，官方戳一戳唤醒主 Agent/LLM 的行为不受影响。
...
| `VECTOR_DIMENSION` | 必须与 Embedding 输出长度一致，默认 1024 |
...
先在 NekroAgent WebUI 登录任意已启用账户（普通用户或超级管理员），然后可直接输入裸地址：
...
控制台的 HTML/CSS/JavaScript 静态外壳可匿名加载，但不包含表情包、统计或配置等业务数据；所有 `/api/*` 读写、预览和管理接口仍由服务端要求有效且已启用的 NA 登录账户。普通登录用户与超级管理员都可以使用上传、编辑、删除、重分析和向量重建功能。HTTP 401 表示未登录或 Token 失效，页面会显示“前往 NA 登录”；若 NA 其他上层策略返回 HTTP 403，页面不会清除当前登录状态。
...
- **戳一戳不阻断官方管道**：poke matcher 使用 `block=False`，插件表情回复与 NekroAgent 官方戳一戳唤醒 LLM 机制并行生效；没有候选、仍在冷却或发送失败时插件侧保持静默。
